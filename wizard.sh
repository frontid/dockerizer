...
echo ''
echo ''
echo -e "\e[47mwizard.sh es para definir un nuevo entorno para un proyecto. Si lo que quieres es arrancar un proyecto ya configurado tienes que usar wizard.sh"
...
if [ ! -f ".dockerizer-project.ini" ] && [ ! -f "_html/.dockerizer-project.ini" ]; then
  ...
  while ! { test "$run_from_conf_file" = 'y' || test "$run_from_conf_file" = 'n'; }; do
    ...
    if [ -f "_html/.dockerizer-project.ini" ]; then
      conf_file_location="_html/.dockerizer-project.ini"
